...
use Modules\Inventory\Jobs\CreateWarehouse;
use Modules\Inventory\Jobs\DeleteWarehouse;
use Modules\Inventory\Jobs\UpdateWarehouse;
...
class Warehouses extends Controller
{
    ...
    {
        $inventory_items = InventoryItem::where('warehouse_id', $warehouse->id)->get();

        $items = [];

        foreach ($inventory_items as $inventory_item) {
            $item = Item::where('id', $inventory_item->item_id)->first();
        ...
        return view('inventory::warehouses.show', compact('warehouse', 'items'));
    }
